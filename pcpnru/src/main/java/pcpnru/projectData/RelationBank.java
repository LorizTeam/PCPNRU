...
public class RelationBank {
	...
	public boolean UpdateStatusReceive(List allList) throws IOException, Exception{
		...
		for(Iterator IterList = allList.iterator();IterList.hasNext();){
			...
			sqlQuery = "update receivehd set approve_tobank = '"+detail.get(3)+"' "
			+ "where docno = '"+detail.get(0)+"' ";
			if(!detail.get(1).equals("")) sqlQuery += " and projectcode = '"+detail.get(1)+"' ";
			if(!detail.get(2).equals("")) sqlQuery += " and project_year = '"+detail.get(2)+"' ";
