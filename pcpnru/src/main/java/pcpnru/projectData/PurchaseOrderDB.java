...
public class PurchaseOrderDB {
	...
	public List GetPRList(String docno,String year) 
			throws Exception { //30-05-2014
				List PRList = new ArrayList();
				String itemno = "",description = "" ,qty = "0", amount = "0", amounttotal = "0", amt_budget = "", frombalance = "", rocking_budget = "", balance = "", docdate = "",remark = "", approve_status = "";
				...
				try {
					...
					String sqlStmt = "SELECT a.po_docno,a.`year`,a.itemno,a.numthai_itemno,a.description,a.qty,a.price,a.numthai_qty,a.unit,a.remark,a.create_by,a.create_datetime " +
								"FROM po_dt a WHERE " ;
					if(!docno.equals("")) sqlStmt = sqlStmt+ "po_docno = '"+docno+"' and "; 
					if(!year.equals("")) sqlStmt = sqlStmt+ "year = '"+year+"' "; 
	...
	public String AddPODT(String pr_docno, String po_docno, String year)  throws Exception{
		...
		if(po_docno.equals("")){
			sqlStmt = "SELECT max(po_docno) as lno FROM po_hd "+
					"WHERE year = '"+year+"' ";
			//System.out.println(sqlStmt);
			pStmt = conn.createStatement();
			rs = pStmt.executeQuery(sqlStmt);		
		...
		sqlStmt = "INSERT INTO po_dt(po_docno,year,itemno,description,qty) " +
		"SELECT '"+po_docno+"',year,itemno,product_code,qty FROM record_approve_dt WHERE docno = '"+pr_docno+"' and year = '"+year+"' ";
		//System.out.println(sqlStmt);
		pStmt = conn.createStatement();
		pStmt.executeUpdate(sqlStmt);
		pStmt.close();
	...
	public void UpdatePOPrice(String po_docno, String year, String itemno, String qty, String amount, String remark)  throws Exception{
		conn = agent.getConnectMYSql();
		...
		String sqlStmt = "UPDATE po_dt set qty = '"+qty+"', price = '"+amount+"' , remark = '"+remark+"', create_date = now() " +
				"WHERE po_docno = '"+po_docno+"' and year = '"+year+"' and itemno = '"+itemno+"' ";
		//System.out.println(sqlStmt);
		pStmt = conn.createStatement();
		pStmt.executeUpdate(sqlStmt);
		pStmt.close();
	...
	public void DeletePRDetail(String po_docno, String year)  throws Exception{
		...
		String sqlStmt = "DELETE FROM po_dt "+
				"WHERE po_docno = '"+po_docno+"' and year = '"+year+"' ";  
		//System.out.println(sqlStmt);
		pStmt = conn.createStatement();
		pStmt.executeUpdate(sqlStmt);
	...
	public boolean chk_pr(String docno, String year) throws Exception {
		...
		try {
			...
			String sqlStmt = "SELECT docno FROM record_approve_dt "+
					"WHERE docno = '"+docno+"' and year = '"+year+"' group by docno ";
			//System.out.println(sqlStmt);
			pStmt = conn.createStatement();
			rs = pStmt.executeQuery(sqlStmt);		
			while (rs.next()) {
