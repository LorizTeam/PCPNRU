...
public class GrpGcostcodeMasterDB {
	...
	public List GetGrpGCostCodeMasterList() 
	throws Exception { //30-05-2014
		List grpGcostCodeMasterList = new ArrayList();
		String gcostcode_r = "", gcostcode_c = "", grp_gcostcode = "",grp_gcostname = "", project_code = "", grp_costyear = "", amount = "0"; 
	...
	public List GetCostCodeList_C(String project_code, String grp_costyear) 
			throws Exception { //30-05-2014
				...
				try {
					...
					while (rs.next()) {
						...
						if (rs.getString("gcostcode_name") != null) 		gcostcode_name = rs.getString("gcostcode_name"); else gcostcode_name = "";
					
						amount_c		= rs.getString("amount");
						amount_c 		= df2.format(Float.parseFloat(amount_c));
							
						CostCodeList_C.add(new GrpGCostCodeMasterModel(project_code, project_name, gcostcode, gcostcode_name, amount_c));
					}
	...
	public List GetGrpCostCodeList(String project_code, String grp_costyear) 
			throws Exception { //30-05-2014
	...
	public void AddGrpGCostCodeMaster(String project_code, String groupcostcode_c, String grp_gcostcode, String grp_gcostname,
		...
		String sqlStmt = "INSERT IGNORE INTO `grp_gcostcode_master` (project_code, gcostcode_c, grp_gcostcode, grp_gcostname, grp_costyear, qty, amount, amounttotal, datetime) "
				+ "VALUES ('"+project_code+"','"+groupcostcode_c+"','"+grp_gcostcode+"','"+grp_gcostname+"','"+grp_costyear+"','"+qty+"','"+amount+"','"+amounttotal+"', now())";
		//System.out.println(sqlStmt);
		pStmt = conn.createStatement();
	...
	public void UpdateCostCodeMaster(String project_code, String groupcostCode, String groupcostName, String groupcostCodeHD, String standardprice, String fundprice, String amount)  throws Exception{
		conn = agent.getConnectMYSql();
		...
		String sqlStmt = "UPDATE groupcostcode_master set gcostcode = '"+groupcostCode+"', gcostcode_name = '"+groupcostName+"',gcostcode_standardprice = '"+standardprice+"' "
				+ ",gcostcode_fundprice = '"+fundprice+"', amount = '"+amount+"', datetime = now() " +
	...
	public String SelectGrpDocNo(String project_code,String grp_costyear) throws Exception {
		...
		try {
			...
			String sqlStmt = "SELECT max(grp_gcostcode) as lno FROM grp_gcostcode_master "+
					"WHERE project_code = '"+project_code+"' and grp_costyear ='"+grp_costyear+"' GROUP BY grp_gcostcode ";
			//System.out.println(sqlStmt);
			pStmt = conn.createStatement();
			rs = pStmt.executeQuery(sqlStmt);		
			while (rs.next()) {
	...
	public String SelectUpdateDocNo(String project_code,String type_gcostcode) throws Exception {
		...
		try {
			...
			String sqlStmt = "SELECT max(gcostcode) as lno FROM groupcostcode_master "+
					"WHERE project_code = '"+project_code+"' and type_gcostcode ='"+type_gcostcode+"' ";
			//System.out.println(sqlStmt);
			pStmt = conn.createStatement();
			rs = pStmt.executeQuery(sqlStmt);		
			while (rs.next()) {
